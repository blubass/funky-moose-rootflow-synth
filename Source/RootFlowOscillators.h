...
/**
 * RootFlow Oscillator
 * Support for Sine, Saw, and PWM Pulse waves with Anti-Aliasing (PolyBLEP).
 */
...
public:
    ...
    float nextSample()
    {
        ...
        else if (targetWave == Waveform::Pulse)
        {
            ...
            // Anti-Aliasing for rising edge (at phase = 0)
            out += polyBlep(phase);

            // Anti-Aliasing for falling edge (at phase = pulseWidth)
